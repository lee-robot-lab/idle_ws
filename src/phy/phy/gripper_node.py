...
class GripperNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.position_drop_detection = bool(
            declare_typed(self, "position_drop_detection", True)
        )
    ...
    def _on_timer(self) -> None:
        ...
        if self._state == GripperState.CLOSING:
            ...
            block_gone = is_drop_detected(
                q=q,
                tau=tau,
                q_cmd=q_cmd,
                tau_drop_threshold=self.tau_drop_threshold,
                position_drop_detection=self.position_drop_detection,
            )
            if block_gone:
                self._state = GripperState.OPEN
