# ================================================================
# stage2/color_net_v2.py
# 설명: ColorNet v2 — tiny CNN → color_logit(4-class) + is_target(binary) 동시 출력.
# 사용법: from stage2.color_net_v2 import ColorNetV2
# ================================================================
import torch
...
class ColorNetV2(nn.Module):
    """이미지 크롭(64×64) + tiny CNN → (color_logit(B,N,4), is_target(B,N,1))."""
    ...
    def __init__(self, crop_size: int = 64, num_colors: int = 4):
        ...
        self.head_is_target = nn.Linear(64, 1)
    ...
    def forward(self, img: torch.Tensor, xy: torch.Tensor):
        ...
        returns: (color_logits(B,N,4), is_target(B,N,1))
        """
        B, N, _ = xy.shape
    ...
    def assign(
        ...
        """Hungarian 1:1 슬롯-색 매핑. present 슬롯만 대상. 반환: (N,) int, -1=absent."""
