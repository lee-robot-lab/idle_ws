...
`ml`은 독립 `ament_python` 패키지다. ROS 노드는 만들지 않고, Python에서 바로 import해서 쓴다.
...
`stage1_v2`는 ResNet18 backbone을 사용하므로 `torchvision`이 반드시 필요하다.

```bash
cd ~/idle_ws
colcon build --packages-select ml
...
print(ColorNetV2.__name__, RelationScorer.__name__)
print(resolve_path("package://ml/checkpoints/stage1_v2/best.pt"))
PY
```
...
```bash
cd ~/idle_ws
PYTHONPATH=src/ml python3 - <<'PY'
from stage2.color_net_v2 import ColorNetV2
from ml_paths import resolve_path

print(ColorNetV2.__name__)
print(resolve_path("package://ml/checkpoints/color_net_v2/best.pt"))
PY
```

체크포인트는 아직 포함하지 않는다. 나중에 정해지면 `src/ml/checkpoints/...`에 두고 `package://ml/checkpoints/...` 형식으로 참조한다.
