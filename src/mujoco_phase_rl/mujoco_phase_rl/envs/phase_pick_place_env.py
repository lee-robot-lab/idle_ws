...
class PhasePickPlaceEnv(gym.Env):
    ...
    def __init__(
        ...
        self.last_pose_estimate: PoseEstimate | None = None
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        self.grasp_offset_pos = np.zeros(3, dtype=np.float64)
        self.grasp_object_quat = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64)
        self.cached_image_embedding = None
        self.image_embedding_observation_count = 0
    ...
    def _observe(self) -> dict[str, np.ndarray]:
        ...
        return self.observer.observe(pose_estimate, state, image_embedding=image_embedding)

    def _estimate_pose(self) -> PoseEstimate:
        if self.current_task is None:
    ...
    def _image_embedding(self) -> np.ndarray:
        if self.image_embedding_mode == "zeros":
            self.last_image_embedding_status = "zeros"
            return np.zeros(IMAGE_EMBEDDING_SIZE, dtype=np.float32)
        if self.image_embedder is None:
            self.image_embedder = CameraImageEmbedder(
                self.model,
                width=self.image_width,
                height=self.image_height,
            )
        should_render = (
            self.cached_image_embedding is None
            or self.image_embedding_observation_count % self.image_embedding_interval == 0
        )
        if should_render:
            self.cached_image_embedding = self.image_embedder.embed(self.data)
            self.last_image_embedding_status = "camera"
        else:
            self.last_image_embedding_status = "camera_cached"
        self.image_embedding_observation_count += 1
        return self.cached_image_embedding.copy()
    ...
    def _info(
        ...
        info = {
            ...
            "image_embedding_status": self.last_image_embedding_status,
            "image_embedding_interval": self.image_embedding_interval,
