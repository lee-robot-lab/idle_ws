...
def main():
    ...
    def _run(bus):
        # 1) write
        arb_id, data = frame_type18_write_u32(args.host_id, args.can_id, pdef.index, value_u32)
        bus.send_ext(arb_id, data)
        print(f"sent WRITE(Type18): can_id={args.can_id} host_id=0x{args.host_id:02X} "
        ...
        if args.verify:
            ...
            print(f"verify READ(Type17): [{st}] value={val} raw={raw4.hex()}")
